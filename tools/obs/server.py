...
from urllib.request import Request as URLRequest, urlopen as urlopen_req
...
| 文件管理 | 按时间/扩展名排序、在线预览、删除 |
| 公告板 | WebSocket 实时同步、多人协作编辑、保存为文件 |
| 剪贴板粘贴 | Ctrl+V 粘贴图片/文件直接上传 |
...
```bash
# 上传文件
curl --upload-file file.txt http://obs.dimond.top/file.txt

# 下载文件
curl http://obs.dimond.top/file.txt -o file.txt
```
"""
}
...
def register_tool():
    ...
    try:
        req = URLRequest(
            ...
            data=json.dumps(OBS_TOOL_INFO).encode("utf-8"),
